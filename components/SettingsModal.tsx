                ...
                {networks.map(net => {
                    ...
                    <li key={net.id} className={`flex justify-between items-center p-3 rounded-[var(--radius-lg)] border transition-colors cursor-pointer ${isSelected ? 'bg-[var(--color-primary)] text-white border-transparent' : 'bg-white border-[var(--color-border-primary)] hover:bg-gray-50'}`} onClick={() => setSelectedNetwork(net)}>
                        <div className="flex items-center space-x-2">
                            ...
                            {net.webexVerified && (
                               <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 ${isSelected ? 'text-white' : 'text-green-500'}`} viewBox="0 0 24 24" fill="currentColor">
                                   {/* FIX: Replaced invalid 'title' prop with a <title> element for accessibility. */}
                                   <title>Webex Verified</title>
           ...
           <main className="w-full md:w-3/5 overflow-y-auto p-6">
            ...
            <form onSubmit={handleSaveNetwork} className="space-y-4">
              ...
              <div>
               ...
               <div>
                  <label className="block text-sm font-medium text-[var(--color-text-secondary)]">Google Gemini API Key</label>
                  <input type="password" name="geminiApiKey" value={formState.geminiApiKey || ''} onChange={handleInputChange} className="mt-1 block w-full bg-[var(--color-surface-subtle)] border border-[var(--color-border-primary)] rounded-[var(--radius-md)] shadow-sm p-3 text-[var(--color-text-primary)] focus:ring-2 focus:ring-[var(--color-primary)] focus:border-transparent transition" autoComplete="new-password"/>
                  <p className="mt-1 text-xs text-[var(--color-text-secondary)]">Optional. If not provided, the app will use the key from the environment variable.</p>
              </div>
